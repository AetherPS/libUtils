...
void DetourCall::Create(void* address, void* destination)
{
	...
	{
		Logger::Error("[DetourCall] create: \"address\" or \"destination\" NULL (%llX -> %llX)\n", address, destination);
		return;
	}
	...
	Logger::Info("[DetourCall] create: Original absolute location -> %llX\n", StubPtr);
	...
	{
		Logger::Error("[DetourCall] create: an error has occurred allocating trampoline: %X, %llX\n", res, TrampolinePtr);
		return;
	}
	...
	Logger::Success("[DetourCall] Detour written from 0x%llX -> 0x%llX\n", address, destination);
	DetourSet = true;
}
